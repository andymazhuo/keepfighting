...
public class FB1 {
	...
	public static String tenToDifferentBase(int target, int base) {
		StringBuilder sb = new StringBuilder();
		while(target>0) {
			sb.insert(0, target%base);
			target = target/base;
		}
		return sb.toString();
	}
	
	public static void shuffleLetter(char[] input, int start, int end) {
		if(end-start <= 1) {
			return;
		} else {
			swap(input, start, end);
			if((end-start+1)%4==0) {
				shuffleLetter(input, start, (start+end)/2);
				shuffleLetter(input, (start+end)/2 + 1, end);
			} else {
				shuffleLetter(input, start, (start+end)/2+1);
				shuffleLetter(input, (start+end)/2 + 2, end);
			}
		}
	}
	
	private static void swap(char[] array, int start, int end) {
		int left = start + (end-start+3)/4, right = left + (end-start)/2;
		for(int i=0; i<(right-left+1)/2; i++) rotate(array, left, right);
	}

	private static void rotate(char[] array, int left, int right) {
		char temp = array[left];
		for(int i=left; i<right; i++) array[i] = array[i+1];
		array[right] = temp;
	}
	...
	public static void main(String[] args) {
		...
		printTreeByCol(n1);
		**/
		char[] input = "abcde12345".toCharArray();
		shuffleLetter(input, 0 , input.length-1);
