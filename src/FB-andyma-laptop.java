...
public class FB {
	...
	public static int rec(TreeNode root, int[] max){
        ...
        max[0] = Math.max(max[0], Math.max(leftSubtreeMaxSum + root.val + rightSubtreeMaxSum, maxPathAcrossRootToParent));
	...
	public static DoubleLinkedListNode multiplyTwoList(DoubleLinkedListNode a, DoubleLinkedListNode b) {
		...
		while(a != null) {
			...
			if(curr.getPre()==null) curr.setPre(new DoubleLinkedListNode(0));
			curr = curr.getPre();
			a = a.getNext();
		}
	...
	private static void chooseKfromN(int index, int k, StringBuilder buffer, int n) {
		if(k==buffer.length()) {
	...
	public static TreeNode convertBTtoLinkedList(TreeNode root) {
		...
		TreeNode temp = root.left; root.left = null;
	...
	public static int getNumOfPalin(String input) {
		...
		int ans = 0;
		for(int m=0; m<l; m++) { cache[m][m] = true; ans++;}
		
		for(int end=1; end<l; end++) {
			for(int start=0; start<end; start++) {
				if(input.charAt(start)==input.charAt(end) && (start+1==end || cache[start+1][end-1])) {
	...
	public static void main(String[] args) {
		...
		DoubleLinkedListNode a = new DoubleLinkedListNode(1);
		a.setNext(new DoubleLinkedListNode(0){{setNext(new DoubleLinkedListNode(1));}});
		DoubleLinkedListNode b = new DoubleLinkedListNode(2);
		b.setNext(new DoubleLinkedListNode(0){{setNext(new DoubleLinkedListNode(2));}});
		...
		StringBuilder sb = new StringBuilder();
		writeBinaryTree(n9, sb);
		System.out.println(sb.toString());
		
		List<String> inputs = new LinkedList<>();
		for(String s : sb.toString().split(" ")) inputs.add(s);
		
		TreeNode res = deserializeBinaryTree(inputs);
		
		StringBuilder sb1 = new StringBuilder();
		writeBinaryTree(res, sb1);
		System.out.println(sb1.toString());
		**/
		System.out.println(getNumOfPalin("aaaa"));
	}
}
